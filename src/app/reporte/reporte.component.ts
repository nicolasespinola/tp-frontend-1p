...
export class ReporteComponent implements OnInit {
  ...
  filtrar() {
    ...
    if(this.fechas.value.desde && this.fechas.value.hasta){
      this.reporteService.getServiciosParams(params).subscribe(
  ...
  exportpdf(): void
  {
    let columns = [["Fecha", "Profesional", "Cliente", "Presupuesto", "Subcategoria"]];
    let data = [];
    for(const s of this.servicios){
      data.push([s.fechaHora, s.idFichaClinica.idEmpleado.nombreCompleto, 
      s.idFichaClinica.idCliente.nombreCompleto, s.presupuesto, s.idFichaClinica.idTipoProducto.descripcion
      ])
    }
